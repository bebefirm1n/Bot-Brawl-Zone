import discordfrom discord import app_commandsfrom discord.ext import commandsimport jsonimport osimport asyncioimport loggingfrom dotenv import load_dotenv

logging.basicConfig(level=logging.INFO)

try:from keep_alive import keep_aliveKEEP_ALIVE_AVAILABLE = Trueexcept ImportError:KEEP_ALIVE_AVAILABLE = False

load_dotenv()TOKEN = os.getenv("TOKEN_BOT")

LIEN_INVITATION = ("https://discord.com/oauth2/authorize?""client_id=1495103873373704472""&permissions=4785076768427024""&integration_type=0""&scope=bot+applications.commands")
...
DUREE_CHOIX = 60          # secondes laissées au membre pour choisir un typeDELAI_NETTOYAGE = 3       # secondes d'attente avant de vérifier si un vocal temp est videDELAI_SUPPRESSION_MSG = 20  # secondes avant de supprimer le message une fois le résultat connuCONFIG_PATH = "vocaux_config.json"
...
def charger_configuration():if os.path.exists(CONFIG_PATH):with open(CONFIG_PATH, "r", encoding="utf-8") as f:return json.load(f)return {}

def sauvegarder_configuration():with open(CONFIG_PATH, "w", encoding="utf-8") as f:json.dump(configuration, f, indent=2, ensure_ascii=False)

configuration = charger_configuration()

def config_serveur(guild_id: int) -> dict:"""Retourne (et initialise si besoin) la config d'un serveur."""gid = str(guild_id)if gid not in configuration:configuration[gid] = {"hubs": [], "types": {}, "temp_channels": {}}configuration[gid].setdefault("hubs", [])configuration[gid].setdefault("types", {})configuration[gid].setdefault("temp_channels", {})return configuration[gid]
...
bot = commands.Bot(command_prefix="!", intents=intents, help_command=None)arbre = bot.tree

@bot.eventasync def on_ready():# Vue persistante : nécessaire pour que les boutons du panneau vocal# continuent de fonctionner après un redémarrage du botbot.add_view(PanelVocalView())
...
class ChoixTypeSelect(discord.ui.Select):def init(self, membre: discord.Member, gconf: dict):self.membre = membreoptions = [discord.SelectOption(label=nom,description=f"Limite : {info['limite']} membre(s)" if info["limite"] else "Limite : illimitée",emoji="🔊",)for nom, info in list(gconf["types"].items())[:25]]super().init(placeholder="Choisis le type de vocal à créer...",options=options,min_values=1,max_values=1,)
...
class ChoixTypeView(discord.ui.View):def init(self, membre: discord.Member, gconf: dict):super().init(timeout=DUREE_CHOIX)self.membre = membreself.message: discord.Message | None = Noneself.add_item(ChoixTypeSelect(membre, gconf))
...
async def supprimer_message_plus_tard(message: discord.Message, delai: int = DELAI_SUPPRESSION_MSG):"""Attend delai secondes puis supprime le message (silencieux si déjà supprimé)."""await asyncio.sleep(delai)try:await message.delete()except discord.HTTPException:pass
...
class RenommerVocalModal(discord.ui.Modal, title="Renommer le vocal"):nouveau_nom = discord.ui.TextInput(label="Nouveau nom", max_length=100)
...
class LimiteVocalModal(discord.ui.Modal, title="Changer la limite"):nombre = discord.ui.TextInput(label="Nouvelle limite (0 = illimité)", default="0", max_length=2)
...
class PanelVocalView(discord.ui.View):"""Vue persistante (custom_id fixes) : une seule instance suffit pour tous les vocaux."""
...
async def creer_vocal_temporaire(interaction: discord.Interaction, membre: discord.Member, nom_type: str):gconf = config_serveur(membre.guild.id)type_info = gconf["types"].get(nom_type)if not type_info:await interaction.response.send_message("❌ Ce type de vocal n'existe plus.", ephemeral=True)return
...
@bot.eventasync def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):if member.bot:return
...
@arbre.errorasync def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):if isinstance(error, app_commands.MissingPermissions):msg = "❌ Tu n'as pas la permission d'utiliser cette commande."else:logging.exception("Erreur commande slash", exc_info=error)msg = "❌ Une erreur est survenue en exécutant cette commande."
...
def embed_config(gconf: dict) -> discord.Embed:types_txt = "\n".join(f"🔊 {n} — limite {i['limite'] or '∞'} · nom {'✅' if i.get('modifiable_nom') else '❌'} · limite {'✅' if i.get('modifiable_limite') else '❌'}"for n, i in gconf["types"].items()) or "Aucun type créé"hubs_txt = "\n".join(f"🎙️ <#{cid}>" for cid in gconf["hubs"]) or "Aucun hub créé"
...
class VueBase(discord.ui.View):"""Base commune : restreint le panneau à son auteur et le désactive au timeout."""
...
class TypeModal(discord.ui.Modal, title="Nouveau type de vocal"):nom = discord.ui.TextInput(label="Nom du type", placeholder="Duo", max_length=50)limite = discord.ui.TextInput(label="Limite de membres (0 = illimité)", default="0", max_length=2)modifiable_nom = discord.ui.TextInput(label="Le nom est modifiable ? (oui/non)", default="non", max_length=3)modifiable_limite = discord.ui.TextInput(label="La limite est modifiable ? (oui/non)", default="non", max_length=3)format_nom = discord.ui.TextInput(label="Format du nom ({pseudo} et {type})",default="🔊 {type} de {pseudo}",max_length=100,required=False,)
...
class SelectTypeASupprimer(discord.ui.Select):def init(self, guild_id: int, gconf: dict):self.guild_id = guild_idoptions = [discord.SelectOption(label=n, value=n, emoji="🔊") for n in gconf["types"]][:25]super().init(placeholder="Choisis le type à supprimer...", options=options, min_values=1, max_values=1)
...
class VueSupprimerType(VueBase):def init(self, guild_id: int, auteur_id: int, gconf: dict):super().init(guild_id, auteur_id)self.add_item(SelectTypeASupprimer(guild_id, gconf))
...
class HubModal(discord.ui.Modal, title="Nouveau salon hub"):nom = discord.ui.TextInput(label="Nom du salon", default="➕ Créer un vocal", max_length=100)
...
class SelectSalonExistant(discord.ui.ChannelSelect):def init(self, guild_id: int):self.guild_id = guild_idsuper().init(placeholder="...ou choisis un salon vocal existant à transformer",channel_types=[discord.ChannelType.voice],min_values=1,max_values=1,row=0,)
...
class SelectCategorieHub(discord.ui.ChannelSelect):"""Choix de la catégorie du salon qui sera créé via le bouton 'Créer un nouveau salon'."""
...
class VueNouveauHub(VueBase):def init(self, guild_id: int, auteur_id: int, guild: discord.Guild):super().init(guild_id, auteur_id)self.categorie_id: int | None = Noneself.add_item(SelectSalonExistant(guild_id))if guild.categories:self.add_item(SelectCategorieHub())
...
class SelectHubASupprimer(discord.ui.Select):def init(self, guild_id: int, gconf: dict, guild: discord.Guild):self.guild_id = guild_idoptions = []for cid in gconf["hubs"]:salon = guild.get_channel(int(cid))if salon:options.append(discord.SelectOption(label=salon.name, value=cid, emoji="🎙️"))super().init(placeholder="Choisis le hub à supprimer...", options=options[:25], min_values=1, max_values=1)
...
class VueSupprimerHub(VueBase):def init(self, guild_id: int, auteur_id: int, gconf: dict, guild: discord.Guild):super().init(guild_id, auteur_id)self.add_item(SelectHubASupprimer(guild_id, gconf, guild))
...
class VueConfig(VueBase):def init(self, guild_id: int, auteur_id: int):super().init(guild_id, auteur_id)
...
@groupe_vocal.command(name="config", description="Panneau de configuration interactif (types & hubs) en un seul endroit")@app_commands.checks.has_permissions(manage_channels=True)async def vocal_config(interaction: discord.Interaction):gconf = config_serveur(interaction.guild_id)vue = VueConfig(interaction.guild_id, interaction.user.id)await interaction.response.send_message(embed=embed_config(gconf), view=vue, ephemeral=True)vue.message = await interaction.original_response()
...
class InviteView(discord.ui.View):def init(self):super().init(timeout=None)
...
CMDS = {"vocal config": {"emoji": "⚙️","titre": "/vocal config","categorie": "Staff","desc": "Panneau de configuration interactif tout-en-un.","details": ("Ouvre un panneau avec des boutons pour tout gérer sans taper d'autres commandes : ""créer/supprimer un type, créer/supprimer un hub. Un formulaire s'ouvre pour les infos ""à saisir (nom, limite, si le nom et/ou la limite du vocal seront modifiables...), et un ""menu déroulant pour choisir un salon existant.\n\n""Permission requise : Gérer les salons"),},"panneau vocal": {"emoji": "🎛️","titre": "Panneau dans ton vocal","categorie": "Membres","desc": "Gère ton vocal temporaire directement depuis ses boutons.","details": ("Dès qu'un vocal temporaire est créé, un message avec deux boutons apparaît dans son ""chat : ✏️ Renommer et 👥 Limite.\n\n""Chaque bouton n'est utilisable que si le staff a autorisé cette modification pour ce ""type de vocal (réglable indépendamment pour le nom et pour la limite via /vocal config), ""et uniquement par le créateur du vocal."),},}

def embed_apercu() -> discord.Embed:embed = discord.Embed(title="❓ Aide — Vocaux temporaires",description="Sélectionne une commande dans le menu ci-dessous pour voir son détail.",color=0xFFD700,)embed.add_field(name="🛠️ Staff",value="/vocal config — ⭐ Panneau interactif tout-en-un (types & hubs)",inline=False,)embed.add_field(name="👤 Membres",value="🎛️ Un panneau de boutons (renommer / limite) apparaît directement dans chaque vocal créé.",inline=False,)embed.set_footer(text="Vocaux temporaires • Menu valable 3 minutes")return embed

class AideSelect(discord.ui.Select):def init(self):options = [discord.SelectOption(label="Vue d'ensemble", value="apercu",description="Retour à la liste des commandes", emoji="📋",)]for cle, info in CMDS.items():options.append(discord.SelectOption(label=info["titre"], value=cle,description=info["desc"][:100], emoji=info["emoji"],))super().init(placeholder="Sélectionne une commande pour en savoir plus...",options=options[:25], min_values=1, max_values=1,)
...
class AideView(discord.ui.View):def init(self, auteur_id: int):super().init(timeout=180)self.auteur_id = auteur_idself.message: discord.Message | None = Noneself.add_item(AideSelect())
...
@arbre.command(name="help", description="Affiche l'aide du bot vocaux temporaires")async def help_cmd(interaction: discord.Interaction):view = AideView(interaction.user.id)await interaction.response.send_message(embed=embed_apercu(), view=view, ephemeral=True)view.message = await interaction.original_response()

arbre.add_command(groupe_vocal)

@bot.eventasync def on_message(message: discord.Message):if message.author.bot:return
...
if KEEP_ALIVE_AVAILABLE:keep_alive()
